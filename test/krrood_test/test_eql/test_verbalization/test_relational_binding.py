"""
Tests for nesting a related entity's restriction into the relative clause that introduces it.

When a query binds a secondary entity through a relational attribute of the subject
(``robot.assigned_to == mission``) and separately restricts that entity (``mission.priority > 2``),
the two conditions fold into one relative clause on the subject noun — *"Find a Robot that is assigned
to a Mission with priority greater than 2"* — instead of the disjointed *"whose assigned_to is a
Mission, such that the priority of the Mission is greater than 2"*.
"""
...
@dataclass
class Book:
    """A book with an agentive ``owned_by`` relation, to check the passive subject-relative form."""
...
def test_agentive_binding_stays_passive():
    """The subject is the patient of an agentive relation, so the clause is passive, not active."""
