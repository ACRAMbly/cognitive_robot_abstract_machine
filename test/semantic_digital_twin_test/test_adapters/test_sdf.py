...
from semantic_digital_twin.adapters.sdf import SDFParser
...
aws_warehouse_repository = (
    "https://github.com/aws-robotics/aws-robomaker-small-warehouse-world.git"
)
aws_warehouse_branch = "ros2"
aws_warehouse_world_file = "worlds/small_warehouse/small_warehouse.world"
aws_warehouse_instance_count = 26
...
class TestModelUriResolution:
    ...
    def test_resolves_against_explicit_directory(self, sdf_paths):
        resolver = ModelUriResolver(model_directories=[sdf_paths.mini_models])
        assert resolver.resolve("model://shelf") == os.path.join(
            sdf_paths.mini_models, "shelf"
        )

    def test_resolves_path_inside_model(self, sdf_paths):
        resolver = ModelUriResolver(model_directories=[sdf_paths.mini_models])
        assert resolver.resolve("model://shelf/model.sdf") == os.path.join(
            sdf_paths.mini_models, "shelf", "model.sdf"
        )

    def test_resolves_against_environment(self, sdf_paths, monkeypatch):
        monkeypatch.setenv("GAZEBO_MODEL_PATH", f"/nowhere:{sdf_paths.mini_models}")
        resolver = ModelUriResolver()
        assert resolver.resolve("model://pallet") == os.path.join(
            sdf_paths.mini_models, "pallet"
        )

    def test_explicit_directory_precedes_environment(self, sdf_paths, monkeypatch):
        monkeypatch.setenv("GAZEBO_MODEL_PATH", sdf_paths.directory)
        resolver = ModelUriResolver(model_directories=[sdf_paths.mini_models])
        assert resolver.search_directories()[0] == sdf_paths.mini_models
    ...
    def test_error_lists_searched_directories(self, sdf_paths, monkeypatch):
        ...
        assert sdf_paths.mini_models in str(error.value)

    def test_inferred_from_world_file_location(self, sdf_paths, monkeypatch):
        """
        A world next to its models resolves without any configuration, which is what
        makes a downloaded world parse as it is.
        """
        monkeypatch.delenv("GAZEBO_MODEL_PATH", raising=False)
        resolver = SDFParser.resolver_for_file(sdf_paths.mini_world)
        assert resolver.resolve("model://shelf") == os.path.join(
            sdf_paths.mini_models, "shelf"
        )
...
class TestModelFileSelection:
    ...
    def test_prefers_highest_declared_version(self, sdf_paths):
        parser = SDFParser.from_file(sdf_paths.mini_world)
        model_file = parser.model_file_of_directory(
            os.path.join(sdf_paths.mini_models, "shelf")
        )
        assert os.path.basename(model_file) == "model.sdf"

    def test_reports_directory_without_configuration(self, sdf_paths):
        parser = SDFParser.from_file(sdf_paths.mini_world)
        with pytest.raises(PathResolutionError):
            parser.model_file_of_directory(sdf_paths.directory)
...
class TestSingleModelParsing:
    ...
    def test_body_is_named_after_model_and_link(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.simple_shapes).parse()
    ...
    def test_collisions_are_separate_from_visuals(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.simple_shapes).parse()
        assert len(body_named(world, "link").collision.shapes) == 1

    def test_parses_inertial_properties(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.simple_shapes).parse()
...
class TestJointParsing:
    ...
    def test_revolute_joint_becomes_revolute_connection(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.hinged_door).parse()
        assert isinstance(connection_named(world, "hinge"), RevoluteConnection)
        assert world.validate()

    def test_link_pose_is_relative_to_the_model(self, sdf_paths):
        """
    ...
    def test_axis_is_expressed_in_the_joint_frame(self, sdf_paths):
        """
    ...
    def test_parses_dynamics(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.hinged_door).parse()
        dynamics = connection_named(world, "hinge").dynamics
        assert dynamics.damping == 0.7
        assert dynamics.dry_friction == 0.3

    def test_prismatic_joint_becomes_prismatic_connection(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.drawer).parse()
    ...
    def test_continuous_joint_has_no_position_limits(self, sdf_paths):
        """
...
class TestWorldParsing:
    ...
    def test_world_has_a_root_of_its_own(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
        assert world.root.name.name == "mini_warehouse"
        assert world.validate()

    def test_instantiates_every_active_model(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
    ...
    def test_wrapped_include_takes_its_name_from_the_wrapper(self, sdf_paths):
        """
    ...
    def test_wrapped_include_is_placed_by_the_sibling_pose(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
    ...
    def test_wrapper_pose_composes_with_include_pose(self, sdf_paths):
        """
    ...
    def test_canonical_include_takes_its_name_from_the_include(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
        pallet = [
    ...
    def test_inline_model_is_placed_at_its_pose(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
    ...
    def test_commented_out_model_is_absent(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
        assert all(body.name.prefix != "shelf_003" for body in world.bodies)

    def test_static_model_is_attached_rigidly(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
        shelf = [body for body in world.bodies if body.name.prefix == "shelf_001"][0]
        assert isinstance(shelf.parent_connection, FixedConnection)

    def test_movable_model_keeps_its_degrees_of_freedom(self, sdf_paths):
        world = SDFParser.from_file(sdf_paths.mini_world).parse()
        pallet = [
            body for body in world.bodies if body.name.prefix == "pallet_canonical"
        ][0]
    ...
    def test_instances_of_one_model_do_not_share_bodies(self, sdf_paths):
        """
...
class TestUnsupportedConstructs:
    ...
    def test_unsupported_joint_type(self, sdf_paths):
        with pytest.raises(UnsupportedJointType) as error:
            SDFParser.from_file(sdf_paths.unsupported_joint).parse()
        assert "ball" in str(error.value)

    def test_unsupported_geometry_type(self, sdf_paths):
        with pytest.raises(UnsupportedGeometryType) as error:
            SDFParser.from_file(sdf_paths.unsupported_geometry).parse()
        assert "plane" in str(error.value)

    def test_pose_with_named_frame_attribute(self, sdf_paths):
        with pytest.raises(UnsupportedPoseReference) as error:
            SDFParser.from_file(sdf_paths.named_pose_frame).parse()
        assert "some_frame" in str(error.value)

    def test_pose_with_named_relative_to_attribute(self, sdf_paths):
        with pytest.raises(UnsupportedPoseReference) as error:
            SDFParser.from_file(sdf_paths.named_pose_relative_to).parse()
        assert "some_frame" in str(error.value)

    def test_axis_in_the_parent_model_frame(self, sdf_paths):
        with pytest.raises(UnsupportedAxisReference) as error:
            SDFParser.from_file(sdf_paths.model_frame_axis).parse()
        assert "lift" in str(error.value)

    def test_axis_expressed_in_another_frame(self, sdf_paths):
        with pytest.raises(UnsupportedAxisReference) as error:
            SDFParser.from_file(sdf_paths.expressed_in_axis).parse()
...
def aws_warehouse_directory():
    ...
    if not (checkout / aws_warehouse_world_file).is_file():
        try:
            subprocess.run(
                [
                    "git",
...
def aws_warehouse_world(aws_warehouse_directory):
    ...
    """
    return SDFParser.from_file(
        str(aws_warehouse_directory / aws_warehouse_world_file)
    ).parse()
...
class TestSmallWarehouseWorld:
    ...
    def test_instantiates_every_model(self, aws_warehouse_world):
        ...
        assert len(instance_names) == aws_warehouse_instance_count
