...
annotation together with a generated body, and `get_default_root_entity_specification` returns the same
...
- Build the drawer's default root specification for the same scale `Scale(0.2, 0.4, 0.2)` with
  `Drawer.get_default_root_entity_specification` and store it in a variable named `specification`
...
# specification = Drawer.get_default_root_entity_specification(...)
# free_standing = specification.to_domain_object("free_drawer_body")
```

```{code-cell} ipython3
:tags: [example-solution]
specification = Drawer.get_default_root_entity_specification(scale=Scale(0.2, 0.4, 0.2))
free_standing = specification.to_domain_object("free_drawer_body")
```
